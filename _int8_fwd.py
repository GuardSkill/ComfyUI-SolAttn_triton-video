...
BLOCK = int(os.environ.get("SOL_BLOCK_SIZE", "64"))
GROUP = 32
...
def _forward_int8(
    ...
    threshold,
    scale,
    sink_start: tl.constexpr,
    sink_end: tl.constexpr,
    sink_q_start: tl.constexpr,
    sink_q_end: tl.constexpr,
    ...
    for group_start in range(0, NT, GROUP_SIZE):
        ...
        routed = (
            ...
            | (tl.abs(q_block - block_indices) <= local_blocks)
            | sink_kv
        ) & valid
...
def _forward_int8_ptr(
    q_ptr, k_ptr, kc_ptr, vc_ptr, o_ptr,
    qi_ptr, qs_ptr, ki_ptr, ks_ptr, vi_ptr, vsc_ptr, v_ptr,
    threshold, protected_ptr,
    scale,
    sink_start: tl.constexpr,
    sink_end: tl.constexpr,
    sink_q_start: tl.constexpr,
    ...
    sk_b, sk_t, sk_h,   # bf16 K strides, used by protected exact blocks
    sv_b, sv_t, sv_h,   # bf16 V strides, read only when INT8_PV is off
    H: tl.constexpr,
    D: tl.constexpr,
    NT: tl.constexpr,
    ...
    GROUP_SIZE: tl.constexpr,
    INT8_PV: tl.constexpr,
    MIXED_EXACT: tl.constexpr,
    USE_PROTECTED_MAP: tl.constexpr,
):
    ...
    for group_start in range(0, NT, GROUP_SIZE):
        ...
        sink_kv = (block_indices >= sink_start) & (block_indices < sink_end)
        coordinate = tl.load(
            protected_ptr + q_block * NT + block_indices,
            mask=valid, other=0,
        ).to(tl.int1) if USE_PROTECTED_MAP else False
        routed = (
            (tl.sum(scores, axis=0) / q_len > route_threshold)
            | (tl.abs(q_block - block_indices) <= local_blocks)
            | sink_kv
        ...
        # In mixed mode, local temporal neighbors and conditioning/keyframe
        # blocks stay BF16. Only routed long-range blocks take INT8 QK/PV.
        protected = exact & (
            (tl.abs(q_block - block_indices) <= local_blocks) | sink_kv | q_in_sink | coordinate
        )
        if MIXED_EXACT:
            protected_offsets = tl.where(protected, group_offsets, GROUP_SIZE)
            for _ in range(tl.sum(protected.to(tl.int32))):
                offset = tl.min(protected_offsets)
                block = group_start + offset
                protected_offsets = tl.where(group_offsets == offset, GROUP_SIZE, protected_offsets)
                kv_start = block * BLOCK_SIZE
                k_rows = kv_start + token_offsets
                k_valid = k_rows < T
                kb = tl.load(
                    k_ptr + batch * sk_b + k_rows[:, None].to(tl.int64) * sk_t
                    + head * sk_h + d_offsets[None, :],
                    mask=k_valid[:, None], other=0.0,
                )
                exact_scores = tl.dot(q, kb.T).to(tl.float32) * scale_log2
                exact_scores += tl.where(k_valid[None, :], 0.0, -float("inf"))
                ...
                output = output * alpha[:, None] + tl.dot(exact_probability.to(vb.dtype), vb)
                row_max = new_max

        # --- routed distant exact branch, INT8 QK/PV ---
        distant = exact & ~protected if MIXED_EXACT else exact
        exact_offsets = tl.where(distant, group_offsets, GROUP_SIZE)
        for _ in range(tl.sum(distant.to(tl.int32))):
            offset = tl.min(exact_offsets)
            block = group_start + offset
            exact_offsets = tl.where(group_offsets == offset, GROUP_SIZE, exact_offsets)
            kv_start = block * BLOCK_SIZE
...
def sol_attn_int8(q, k, v, *, scale=None, tau=1.0, sink_blocks=(0, 0), sink_q=(0, 0),
                  local_blocks=1, use_tma=False, int8_pv=True, mixed_exact=False,
                  protected_blocks=None):
    """Sol-Attn with an INT8 exact branch for FP16 or BF16 inputs."""
    scale = q.shape[-1] ** -0.5 if scale is None else float(scale)
    batch, _, heads, head_dim = q.shape
    use_tma = use_tma and _has_tma(q.device) and protected_blocks is None
    if use_tma:
    ...
    if protected_blocks is None:
        protected_blocks = torch.empty(1, device=q.device, dtype=torch.uint8)
        use_protected_map = False
    else:
        if tuple(protected_blocks.shape) != (blocks, blocks):
            raise ValueError(f"protected_blocks must be {(blocks, blocks)}, got {tuple(protected_blocks.shape)}")
        protected_blocks = protected_blocks.to(device=q.device, dtype=torch.uint8).contiguous()
        use_protected_map = True

    kc, vc, threshold, qi, qs, ki, ks, vi, vsc = fused_preprocess(
        q, k, v, tau=tau, scale=scale, tokens=tokens,
        int8_pv=int8_pv,
    )
    if use_tma and mixed_exact:
        raise NotImplementedError("mixed_exact currently targets the SM80/SM89 pointer kernel")
    if vi is None:
    ...
    if not use_tma:
        ...
        _forward_int8_ptr[grid](
            ...
            USE_PROTECTED_MAP=use_protected_map,
        )
